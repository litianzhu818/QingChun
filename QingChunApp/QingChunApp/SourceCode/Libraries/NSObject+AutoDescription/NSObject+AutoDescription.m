...
- (NSString *)keyValueAutoDescription
{
	...
	id associatedObject = objc_getAssociatedObject(self, CFBridgingRetain(currentQueue));
	if (associatedObject) {
		return @"(self)";
	}

	objc_setAssociatedObject(self, CFBridgingRetain(currentQueue), result, OBJC_ASSOCIATION_RETAIN);
	...
	objc_setAssociatedObject(self, CFBridgingRetain(currentQueue), nil, OBJC_ASSOCIATION_RETAIN);
